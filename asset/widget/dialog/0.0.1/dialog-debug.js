 ...
 * @class Dialog
 */
define("widget/dialog/0.0.1/dialog-debug", [ "./ysDialog-debug.css" ], function(require, exports, module) {
    /*----------------和业务有关的公用函数-----------------*/
    if (window.ysDialog) return;
    window.ysDialog = {
        eventList: []
    };
    require("./ysDialog-debug.css");
    var isArray = $.isArray;
    var isIE = util.browser.isIE, IE6 = util.browser.isIE6, isCompat = $.browser.isCompat, isObj = $.isPlainObject, useFixed = !isIE || !IE6 && isCompat;
    //滚动时，IE7+（标准模式）及其它浏览器使用Fixed定位
    ...
    var dftCfg = {
        ...
        //取消按钮的提示文本
        msgCls: "ys-content",
        //消息框内容的class名称，用于自定义验尸官，默认为ys-content,仅在iframe:false时有效
        minBtn: false,
        //是否显示最小化按钮，默认不显示
        minTxt: "最小化",
        //最小化按钮的提示文本
    ...
    (function() {
        ...
        var container = [ /*遮罩*/
        "<div id='maskLevel' style='" + maskStyle + ";z-index:10000;'></div>", IE6 ? "<iframe id='maskIframe' src='' style='" + maskStyle + ";z-index:9999;filter:alpha(opacity=0);opacity:0'></iframe>" : "", /*窗体*/
        ...
        isIE ? "<div id='ys-shadow' style='position:absolute;z-index:10000;background:#808080;" + "filter:alpha(opacity=80) progid:DXImageTransform.Microsoft.Blur(pixelradius=2);display:none'></div>" : "" ].join("");
        ...
        var ys_ico = ys_hTool.childNodes;
        //右上角的图标
        var dragVar = {};
        /*窗口的最大化最小化核心功能实现*/
        var cur_state = "normal", cur_cord = [ 0, 0 ];
        //cur_cord记录最大化前窗口的坐标
        ...
        var doMin = function() {
            ...
            setWinSize(0, $(ys_headbox).height(), cur_cord);
        };
        var doNormal = function(init) {
            //init=true,弹出时调用该函数
        ...
        //保存当前页面的实际大小
        /*事件绑定部分*/
        var bindEl = ys_head.setCapture && ys_head;
        //绑定拖放事件的对象，只有Ie下bindEl有效
        /*窗体透明度控制*/
        ...
        $(ys_head).on("mousedown", function(e) {
            if (curCfg.disableDrag) return false;
            // 禁止拖动 2013-03-09
            if ((e.srcElement || e.target).parentNode == ys_hTool) return false;
            //点击操作按钮不进行启用拖动处理
        ...
        var maskVisible = function(visible) {
            ...
            if (visible === false) {
                ...
                return $(maskEl).css("display", "none");
            } else {
        ...
        var setWinSize = function(w, h, pos) {
            if (ys_win.style.display == "none") return;
            //当前不可见则不处理
            /*默认使用配置的宽高*/
            ...
            //保存当前窗口位置信息
            if (!curCfg.noHeight) {
                $(ys_body).css("height", h - $(ys_headbox).height() - $(ys_btn).height() - $(ys_bottom).height() + "px");
            } else {
                $(ys_body).css("height", "auto").css("max-height", getWinSize()[1] * .6 + "px");
            }
            isCompat && IE6 && curCfg.iframe && $(getPage()).css({
                height: ys_body.clientHeight
            });
        };
        ...
        var winVisible = function(visible) {
            ...
            ys_head.className = "ys-tc" + (curCfg.titleBar ? "" : " ys-ttc");
            //无标题栏
            ys_hText.innerHTML = curCfg.title;
            //标题
            ...
            ys_body.innerHTML = !curCfg.iframe ? '<div class="' + curCfg.msgCls + '">' + curCfg.content + "</div>" : "<iframe style='" + ifmStyle + "' border='0' frameborder='0' src='" + curCfg.content + "'></iframe><div style='" + ifmStyle + ";background:#000;opacity:0.1;filter:alpha(opacity=10);display:none'></div>";
            //内容
            (function(el, obj) {
                for (var i in obj) try {
                    el[i] = obj[i];
                } catch (e) {}
            })(ys_body.firstChild, curCfg.iframe);
            //为iframe添加自定义属性
            ys_body.className = "ys-body " + curCfg.icoCls;
            //图标类型
            $(ys_btn).css("display", (ys_btnContent.innerHTML = joinBtn(mkBtn(curCfg.btn))) ? "" : "none");
            //没有按钮则隐藏
            !curCfg.useSlide && curCfg.showShadow && $(ys_shadow).css("display", "");
            $(ys_win).css("display", "");
            doNormal(true);
            filterWin(curCfg.useSlide ? 0 : 1);
            //此处使用filter同时可以解决IE非标准模式下有时下边会出现1px空白，使内容与下部不衔接的问题
            /*渐变方式显示*/
        ...
        //初始化
        var init = function() {
            ys_wins = [ ys_win ].concat(curCfg.showShadow ? ys_shadow : "");
            //是否使用阴影
            maskVisible();
            winVisible();
        };
        ...
        $.extend(ysDialog, {
            ...
            show: function(args, fargs, show, inShow) {
                ...
                $.extend(curCfg, ysDialog.setDefaultCfg(), $.extend({}, fargs, o));
                //先还原默认配置
