        ...
        $.ajax(self = $.extend(true, {
            ...
            success: function( response ) {
                var valid = response === true || response === "true",
                    errors, message, submitted,msg,rtn;
    ...
    jQuery.validator.setDefaults({
        ...
        success: function (label, element) {
            ...
            var $ele = $(element),
                pos = $ele.position(),
                dom = $ele[0],
                margin_top = Number(/\d+/.exec(getStyle(dom,'marginTop'))),
                padding_bottom = Number(/\d+/.exec(getStyle(dom,'paddingBottom'))),
                margin_left = Number(/\d+/.exec(getStyle(dom,'marginLeft')));
